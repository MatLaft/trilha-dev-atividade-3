...
function addClass(classId, name, teachersId) {
    if(!isClassRegistered(classId)) {
        ...
        classes.push({ id: classId, name:name, teachers: teachers, students: [] });
    }
}

function addStudent(studentId, name, email, age, classId) {
    if(!isStudentRegistered(studentId)) {
        const student = { id: studentId, name: name, email: email, age: age, classId: classId };
...
function removeClass(classId) {
    // Implemente a lógica.
    if(isClassRegistered(classId)) {
        const classIndex = classes.findIndex(classe => classe.id === classId);
        students.forEach(studentsAux => {
            studentsAux.classId = null;
...
expect(students[0].classId, null, 'Deve remover turma do aluno');
